...
impl RealtimePlanRunner {
    ...
    pub fn process_ready(
        ...
        self.process_ready_with_clock(work_budget_frames, crate::timing::monotonic_timestamp_ns)
    }
...
mod tests {
    ...
    type RunnerFixture = (
        ...
        Vec<crate::runtime::PlanEdgeReceiver>,
        Vec<NodeId>,
    );
    ...
    fn runner_fixture() -> RunnerFixture {
        ...
        registry
            .register(Arc::new(WorkerSinkFactory("test.sink.left")))
            .unwrap();
        registry
            .register(Arc::new(WorkerSinkFactory("test.sink.right")))
            .unwrap();
    ...
    fn given_two_ready_sources_when_processed_then_each_source_dispatches_independently() {
        ...
        let delivered = workers
            ...
            .map(|frame| match frame {
                PlanEdgeFrame::Exclusive(frame) => frame.frame().source_id.0,
                PlanEdgeFrame::Shared(frame) => frame.frame().source_id.0,
            })
            .collect::<Vec<_>>();
