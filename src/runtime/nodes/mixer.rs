...
use crate::frame::{
    AudioBufferPool, AudioFrame, SampleFormat, SourceId, StreamId, POOL_SLOT_SAMPLES,
};
...
const MIXER_MONO_DOWNMIX_SCALE: f32 = 0.5;
                    ...
                    (MONO_CHANNEL_COUNT, STEREO_CHANNEL_COUNT) => {
                        for pair in src.chunks_exact(2) {
                            if accum.len() < MIXER_ACCUM_PREALLOC_SAMPLES {
                                accum.push(MIXER_MONO_DOWNMIX_SCALE * (pair[0] + pair[1]));
                            }
                        }
                    }
