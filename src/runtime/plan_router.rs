...
impl EdgeTelemetry {
    ...
    fn observe_delivery(&self, queued: &QueuedPlanEdgeFrame, delivered_at_ns: u64) {
        ...
        let shares_receive_clock = queued.frame.lineage().is_some_and(|lineage| {
            lineage.clock_id == crate::timing::PROCESS_MONOTONIC_CLOCK_DOMAIN_ID
        });
...
mod tests {
    use super::*;
    use crate::frame::{ClockDomainId, SessionId, SourceId, StemId, StreamId};
    ...
    fn given_foreign_clock_timestamp_when_delivered_then_source_latency_is_not_fabricated() {
        ...
        let queued = QueuedPlanEdgeFrame {
            frame: PlanEdgeFrame::LineagedExclusive(frame),
            enqueued_at_ns: 150,
        };
