...
impl EndpointPrepareContext {
    ...
    pub fn with_derived_contract(
        mut self,
        signal_spec: SignalSpec,
        media: MediaCaps,
        edge_contract: EdgeContract,
    ) -> Self {
        self.derived_signal_spec = Some(signal_spec);
        self.derived_media = Some(media);
        self.derived_edge_contract = Some(edge_contract);
        self
    }
    ...
    pub const fn derived_signal_spec(&self) -> Option<&SignalSpec> {
        self.derived_signal_spec.as_ref()
    }

    pub const fn derived_media(&self) -> Option<&MediaCaps> {
        self.derived_media.as_ref()
    }

    pub const fn derived_edge_contract(&self) -> Option<&EdgeContract> {
        self.derived_edge_contract.as_ref()
    }
}
...
#[cfg(test)]
mod tests {
    use crate::frame::{EndpointId, RouteId, SampleFormat, SampleSpec, SessionId, StemId};
    use crate::graph::{NodeConfig, PrepareContext};
