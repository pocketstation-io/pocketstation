...
use crate::session::{
    DerivedStreamHandle, EndpointHandle, Operator, RouteId, SessionError, StemHandle,
};
...
impl<Signal: StreamSignal> Stream<Signal> {
    ...
    pub fn through<Output: StreamSignal>(
        ...
        let derived = match self.origin {
            StreamOrigin::Stem(stem) => stem.through_ports(
                operator.operator,
                Some(operator.input_port),
                Some(operator.output_port),
            )?,
            StreamOrigin::Derived(stream) => stream.through_ports(
                operator.operator,
                Some(operator.input_port),
                Some(operator.output_port),
            )?,
        ...
        Ok(Stream {
            origin: StreamOrigin::Derived(derived),
    ...
    pub fn send(&self, endpoint: EndpointHandle) -> Result<RouteId, TypedStreamError> {
        match &self.origin {
            StreamOrigin::Stem(stem) => stem.send(endpoint).map_err(Into::into),
            StreamOrigin::Derived(stream) => stream.send(endpoint).map_err(Into::into),
        }
    }
}
