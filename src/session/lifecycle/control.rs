...
pub(super) fn validate_source_topology(
    ...
    let microphone_sources = prepared
        ...
        .count();
    let system_mix_sources = prepared
        .spec
        .stems()
        .iter()
        .filter(|stem| matches!(stem.source(), Source::SystemMix))
        .count();
...
mod tests {
    ...
    fn given_supported_source_compositions_when_validated_then_each_is_accepted() {
        ...
        assert!(source_topology_has_input(2, 3, 1, 0));
