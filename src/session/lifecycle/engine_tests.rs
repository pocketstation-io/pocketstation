...
use crate::endpoint::{
    ...
    EndpointDriverObservations, EndpointDriverRegistryError, EndpointFailure, EndpointGroupId,
    EndpointPortInput, EndpointPreparationGroup, EndpointStartGate, PreparedEndpointDriver,
...
use crate::session::{
    ApplicationSelector, CaptureBackendSet, DeviceSelector, EndpointConfiguration,
    EndpointExtensionRegistrationError, OperatorId, PolledAudioEndpoint, PolledAudioEndpointConfig,
    PolledAudioPollError, Session, SessionCompileError, SessionEngineBuilder,
...
struct TestEndpointControl {
    ...
    observed_sample_rate_hz: AtomicU64,
}
...
impl EndpointDriverFactory for TestEndpointFactory {
    ...
    fn prepare(
        ...
        for input in &inputs {
            let crate::endpoint::EndpointReceiver::Audio { sample_spec, .. } = input.receiver()
            else {
                panic!("audio endpoint received a signal input");
            };
            self.control
                .observed_sample_rate_hz
                .store(u64::from(sample_spec.sample_rate_hz), Ordering::Relaxed);
...
fn prepare_context_16khz() -> PrepareContext {
...
#[test]
fn given_16khz_session_when_started_then_compiled_endpoint_contexts_preserve_declared_rate() {
    let control = Arc::new(TestEndpointControl::default());
    let mut builder =
        SessionEngineBuilder::new(prepare_context_16khz(), 8, SessionStartOptions::default())
            .expect("16 kHz engine builder");
    ...
    let engine = builder.build().expect("16 kHz engine build");
    let capture = TestCaptureBackend::default();
    let mut running = engine
        .start(
            product_session(),
        ...
        .expect("16 kHz Session start");

    assert_eq!(
        control.observed_sample_rate_hz.load(Ordering::Relaxed),
        16_000
    );
    assert_eq!(control.stereo_inputs_total.load(Ordering::Relaxed), 3);
    assert_eq!(control.mono_inputs_total.load(Ordering::Relaxed), 3);
...
fn given_duplicate_or_conflicting_operator_when_registering_then_builder_fails_typed() {
    ...
    let duplicate = duplicate_builder.register_audio_endpoint_driver(
        OperatorId::new(CONNECTOR_OPERATOR_ID),
        NodeTypeId::from(CONNECTOR_NODE_TYPE_ID),
        endpoint_factory(&control),
    );
    assert!(matches!(
        duplicate,
        Err(EndpointExtensionRegistrationError::Driver(
            EndpointDriverRegistryError::Duplicate { .. }
        ))
    ));

    let conflict = duplicate_builder.register_audio_endpoint_driver(
        OperatorId::new(CONNECTOR_OPERATOR_ID),
        NodeTypeId::from(BROWSER_NODE_TYPE_ID),
        endpoint_factory(&control),
    );
    assert!(matches!(
        conflict,
        Err(EndpointExtensionRegistrationError::Driver(
            EndpointDriverRegistryError::OperatorNodeTypeConflict { .. }
        ))
    ));
}
