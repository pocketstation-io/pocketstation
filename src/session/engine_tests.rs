...
use crate::endpoint::{
    ...
    EndpointDriverInput, EndpointDriverObservations, EndpointDriverRegistryError, EndpointFailure,
...
use crate::session::{
    ...
    SessionCompileError, SessionEngineBuilder, SessionEngineStartError, SessionStartError,
    SessionStartOptions, Source, BROWSER_NODE_TYPE_ID, BROWSER_OPERATOR_ID, CONNECTOR_NODE_TYPE_ID,
...
impl EndpointDriverFactory for TestEndpointFactory {
    fn prepare(
        ...
        for input in &inputs {
            self.control.observed_sample_rate_hz.store(
                u64::from(
                    input
                        .context()
...
fn given_duplicate_or_conflicting_operator_when_registering_then_builder_fails_typed() {
    ...
    assert!(matches!(
        duplicate,
        Err(EndpointDriverRegistryError::Duplicate { .. })
    ));
