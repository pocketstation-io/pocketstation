    ...
    fn next(
        ...
        let envelope = SignalEnvelope::untracked(
            SignalPayload::Custom {
                signal_id: crate::graph::SignalId::new(SIGNAL_ID),
                bytes: sequence.to_le_bytes().to_vec(),
            },
            sequence,
        )
        .map_payload(
...
struct PreparedLifecycleEndpoint {
    input: DerivedEndpointDriverInput,
...
impl EndpointDriverFactory for LifecycleEndpointFactory {
    fn prepare(
        &self,
        _inputs: Vec<crate::endpoint::EndpointDriverInput>,
    ) -> Result<Box<dyn PreparedEndpointDriver>, EndpointFailure> {
        Err(EndpointFailure::new(
            EndpointFailureStage::Prepare,
            "typed lifecycle endpoint requires signal input",
        ))
    }

    fn prepare_derived(
        &self,
        mut inputs: Vec<DerivedEndpointDriverInput>,
    ) -> Result<Box<dyn PreparedEndpointDriver>, EndpointFailure> {
        let input = inputs.pop().ok_or_else(|| {
            EndpointFailure::new(EndpointFailureStage::Prepare, "missing signal input")
        })?;
        assert!(input.context().node_prepare_context().is_none());
        assert!(input.context().signal_route_context().is_some());
...
impl PreparedEndpointDriver for PreparedLifecycleEndpoint {
    fn start(
        ...
        let (mut receiver, _) = input.into_parts();
...
struct PreparedAudioEndpoint {
    input: crate::endpoint::EndpointDriverInput,
    control: Arc<EndpointControl>,
    expected_source_id: Arc<AtomicU64>,
}
...
impl EndpointDriverFactory for AudioEndpointFactory {
    fn prepare(
        &self,
        mut inputs: Vec<crate::endpoint::EndpointDriverInput>,
    ) -> Result<Box<dyn PreparedEndpointDriver>, EndpointFailure> {
        let input = inputs.pop().ok_or_else(|| {
            EndpointFailure::new(EndpointFailureStage::Prepare, "missing audio input")
        })?;
        assert!(input.context().node_prepare_context().is_some());
        let source = input.context().signal_route_context().ok_or_else(|| {
            EndpointFailure::new(
                EndpointFailureStage::Prepare,
        ...
        self.expected_source_id
            .store(source.source_id().0, Ordering::Relaxed);
        ...
        let worker = std::thread::spawn(move || {
            ...
            while !worker_stop.load(Ordering::Acquire) {
                if let Some(frame) = receiver.try_recv() {
                    ...
                    assert_eq!(
                        frame.lineage().expect("external audio lineage").clock_id,
                        ClockDomainId(9)
                    );
                    worker_control
                        .received_total
...
fn given_typed_only_session_when_one_branch_saturates_then_other_branch_and_shutdown_remain_real() {
    ...
    builder
        ...
        .unwrap();
    builder
        .register_endpoint_definition(Arc::new(EndpointDefinition))
        .unwrap();
    builder
...
fn given_external_pcm_source_when_session_runs_then_audio_uses_bounded_ingress_with_source_identity(
    ...
    builder
        ...
        .unwrap();
    builder
        .register_endpoint_definition(Arc::new(AudioEndpointDefinition))
        .unwrap();
    builder
        .register_endpoint_driver(
