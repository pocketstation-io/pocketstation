...
pub enum CompileError {
    ...
    MediaMismatch {
        ...
        from: String,
        #[doc = "Identifies the destination represented by `MediaMismatch`."]
        to: String,
    },
    ...
    SignalMismatch {
        ...
        from: String,
        #[doc = "Identifies the destination represented by `SignalMismatch`."]
        to: String,
    },
    #[error(
...
mod tests {
    ...
    struct BinarySourceFactory;
    impl NodeFactory for BinarySourceFactory {
        fn descriptor(&self) -> NodeDescriptor {
            test_descriptor(
                "source.binary",
                Vec::new(),
    ...
    struct BinarySinkFactory;
    impl NodeFactory for BinarySinkFactory {
        fn descriptor(&self) -> NodeDescriptor {
            test_descriptor(
                "sink.binary",
                vec![port(
    ...
    fn test_registry() -> NodeRegistry {
        ...
        registry.register(Arc::new(BinarySourceFactory)).unwrap();
        registry.register(Arc::new(BinarySinkFactory)).unwrap();
    ...
    #[test]
    fn given_unspecified_binary_edge_when_compiled_then_bounded_async_contract_is_derived() {
        let registry = test_registry();
        let mut graph = Pipeline::new();
        let source = graph.add_node("source.binary", NodeConfig::new());
        let sink = graph.add_node("sink.binary", NodeConfig::new());
        graph.connect(source.out("signal"), sink.in_("signal"));
