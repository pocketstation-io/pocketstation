...
// global allocator replaces the NIH counting allocator that was previously
// maintained in `tools/pocketstation-alloccheck`.
//
// This is an integration test (separate binary) so it can declare its own
// #[global_allocator] without conflicting with any other test binary.
...
fn given_hot_path_when_100_frames_then_zero_heap_allocs() {
    ...
    // Pre-allocate caller-owned buffers once.  The hot path only reuses them.
    // encode_buf capacity must be at least OPUS_MAX_PACKET_BYTES so that
    // encode_into never grows it during the measurement window.
    // decode_buf capacity must be at least DEFAULT_SLOT_SAMPLES_MONO_20MS so
    // that decode_into never grows it during the measurement window.
    ...
    // Warm up 10 frames to prime branch predictors, ring internals, and libopus
    // internal SILK decoder state (which allocates lazily on first few frames on
    // Linux glibc; 10 frames is sufficient to exhaust all lazy-init paths before
    // the assert_no_alloc block opens).
    for wu in 0u64..10 {
    ...
    // Hot path: assert_no_alloc enforces zero heap allocation across all 100 frames.
    assert_no_alloc(|| {
        for seq in 1u64..=100 {
