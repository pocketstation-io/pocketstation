...
usage() {
  cat <<'USAGE'
usage: build-android-codec-c.sh \
  --ndk-dir PATH \
  --output-root PATH \
  [--target-dir PATH]

Builds the real pks-codec-c static archive for arm64-v8a and copies it to:

  <output-root>/arm64-v8a/libpks_codec_c.a
...
built_archive="${target_dir}/${rust_target}/release/libpks_codec_c.a"
[[ -f "${built_archive}" ]] || {
  echo "pks-codec-c Android archive was not produced" >&2
  exit 1
}
output_archive="${output_root}/${android_abi}/libpks_codec_c.a"
