...
- Stable constants identify the default recording group, manifest file name, and manifest schema version for SDK projections.
...
- A running Session does not imply that every output file or recording manifest finalized.
- The additive manifest constants do not change the externally constructible `RecordingOutcome` shape.
...
- `given_registered_multistem_recorder_when_host_built_then_receipt_is_retained` — given registered multistem recorder when host built then receipt is retained (`src/session/lifecycle/host.rs:702`; `test-2ca2fe231bf5676fd02b`).
